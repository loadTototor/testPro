...
class Program(tk.Tk):
    def __init__(self):
        ...
        self.path_qr = StringVar()#二维码sn

        self.fa_name = StringVar()
        self.set_init_window()

        self.title("烧录工具_v1.0")  # 窗口名
        self.geometry('534x600+150+50')  # 290 160为窗口大小，+10 +10 定义窗口弹出时的默认展示位置
    ...
    def set_init_window(self):
        ...
        self.help_data_button.grid(row=0, column=1, sticky=W)

        self.program_name_lable = Label(self, text='program file name').grid(row=1, column=0,sticky=W)
        self.program_msg_entry=Entry(self,textvariable=self.fa_name).grid(row=1,column=1, columnspan=2, sticky=EW)

        self.msg_QR_code_lable=Label(self,text='QR code：').grid(row=2, column=0,sticky=W)
        self.msg_entry = Entry(self)
        self.msg_entry.grid(row=2,column=1, columnspan=2, sticky=EW)

        self.program_button = Button(self, text='program',command=self.write_log_to_Text)  # 烧录按钮
        self.program_button.grid(row=2, column=2, sticky=EW)
    ...
    def create_to_pro(self):
        ...
        stderr=subprocess.STDOUT)
            ...
            # p = subprocess.Popen("cmd.exe /"+STM32_file+STM32_exe +' -C port=SWD freq=4000 -w  '+self.path_fa.get()+'-v -rst', stdout=subprocess.PIPE,shell=True,
            #                      stderr=subprocess.STDOUT)
            data=[]#日志信息
